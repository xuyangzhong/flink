...
/** CombineAllProgram. */
public class CombineAllProgram extends TestProgram {

    private final TestOperator testOperator;

    public CombineAllProgram(long dataCount, String remoteFilePath) {
        ...
        public void processElement(StreamRecord<Tuple2<String, Long>> element) throws Exception {
            ...
            StateFutureUtils.combineAll(
                            Arrays.asList(
                                    countState
                                            .asyncValue()
                                            .thenAccept(preCount -> count.set(preCount + 1)),
                                    sumState.asyncValue()
                                            .thenAccept(preSum -> sum.set(preSum + value))))
                    .thenAccept(
                            VOID -> {
    ...
    public static void main(String[] args) throws Exception {
        CombineAllProgram test = new CombineAllProgram(1L, "");
        test.run();
    }
}
