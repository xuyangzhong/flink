...
import org.apache.flink.table.planner.alias.SubQueryAliasClearShuttle;
...
public class SqlToRelConverter {
    ...
    public RelRoot convertQuery(SqlNode query, final boolean needsValidation, final boolean top) {
        ...
        result = result.accept(new SubQueryAliasClearShuttle());
    ...
    private RexNode convertOver(Blackboard bb, SqlNode node) {
        ...
        if (!aggCall.getOperator().allowsFraming()) {
            ...
            if (sqlLowerBound == null && sqlUpperBound == null) {
    ...
    protected void convertPivot(Blackboard bb, SqlPivot pivot) {
        ...
        final Builder<Pair<String, List<RexNode>>> valueList = ImmutableList.builder();
    ...
    private void convertJoin(Blackboard bb, SqlJoin join) {
        ...
        if (checkInvalidSnapshotJoin(tempRightRel, join.getJoinType())) {
            throw SqlUtil.newContextException(
                    join.getParserPosition(), RESOURCE.invalidSnapshotJoin());
        }
    ...
    private boolean checkInvalidSnapshotJoin(RelNode right, JoinType joinType) {
        if (right instanceof LogicalSnapshot) {
            switch (joinType) {
                case RIGHT:
                case FULL:
                    return true;
