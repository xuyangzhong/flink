...
class CalcTest extends TableTestBase {
  ...
  def test(): Unit = {
    ...
    val sql =
      s"""
         | select /*+ BROADCAST(test1) */* from test1 join test2 on test1.a = test2.a
         |""".stripMargin
    util.verifyRelPlan(sql)

//    val set = util.tableEnv.createStatementSet();
//    set.addInsertSql(
//      "insert into sink1 select test1.a from test1 join test2 on test1.a = test2.a")
//    set.addInsertSql(
//      "insert into sink2 select test1.a + 1 from test1" +
//        " join test2 on test1.a = test2.a where test1.a > 5")
//
